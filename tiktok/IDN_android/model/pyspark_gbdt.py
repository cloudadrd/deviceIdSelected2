...
def eval_metric(data_type,prediction,prediction_col="rawPrediction_1",evaluate_auc=True):
    ...
    if evaluate_auc:
        ...
        print('%s AUC: %.6f label_temp=1 and label_temp=2' % (data_type, evaluator.evaluate(prediction)))

        prediction_temp_1 = prediction.where("label_temp=1 or label_temp=3")
        print('%s AUC: %.6f label_temp=1' % (data_type, evaluator.evaluate(prediction_temp_1)))
